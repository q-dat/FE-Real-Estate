    ...
    >
          ...
          <div className="flex items-end justify-between gap-2">
            <div className="min-w-0">
              ...
              <div className="inline-flex max-w-full rounded-md bg-white px-2 py-1.5 shadow-sm">
                <span className="truncate text-sm font-black tracking-tight text-zinc-950 xl:text-base">
                  {priceDisplay}
                </span>
              </div>
            </div>
      ...
      <div className="flex flex-1 flex-col p-2.5 xl:p-3">
        ...
        <div className="mt-2 flex items-start gap-1.5 rounded-md border border-zinc-200 bg-zinc-50 px-2 py-1.5">
          ...
          <p className="line-clamp-2 text-[11px] font-medium leading-relaxed text-zinc-600">
            {location}
          </p>
        </div>
        ...
        <div className="mt-2 grid grid-cols-2 gap-1.5">
          {post.area ? (
            <CompactStat icon={<IoMdExpand size={14} />} label="Diện tích" value={`${post.area} m²`} />
          ) : null}

          {dimensionText ? (
            <CompactStat
              icon={<AiOutlineColumnWidth size={14} />}
              label="Ngang dài"
              value={dimensionText}
            />
          ) : null}
          ...
          {roomText ? (
            <CompactStat
              icon={<MdOutlineBedroomParent size={14} />}
              label="Công năng"
              value={roomText}
            />
          ) : null}
          ...
          {post.floorNumber ? (
            <CompactStat icon={<MdHomeWork size={14} />} label="Số tầng" value={`${post.floorNumber} tầng`} />
          ) : null}
        </div>
        ...
        {pricePerM2 !== null ? (
          <div className="mt-2 flex items-center justify-between gap-2 rounded-md border border-zinc-200 bg-white px-2 py-1.5">
            ...
            <span className="truncate text-[11px] font-black text-zinc-800">
          ...
          <div className="grid grid-cols-2 gap-2 xl:grid-cols-4">
            {sections.map((section, index) => (
              ...
              >
                ...
                <p className="mb-1 text-[9px] font-black uppercase tracking-[0.16em] text-zinc-400">
                  {section.subtitle}
                </p>

                <p className="line-clamp-2 text-sm font-black leading-snug text-zinc-950 group-hover:text-zinc-700">
                  {section.title}
                </p>
              </Link>
            ))}
              ...
              <div className="mb-3 rounded-lg border border-zinc-200 bg-white px-3 py-3 shadow-sm xl:mb-4">
                <div className="flex flex-col gap-3 xl:flex-row xl:items-end xl:justify-between">
                  <div className="min-w-0">
                    ...
                    <h2 className="text-2xl font-black tracking-tight text-zinc-950 xl:text-3xl">
                      {section.title}
                    </h2>
      ...
      <section className="px-2 pb-8 xl:px-desktop-padding xl:pb-12 2xl:px-8">
        <div className="mx-auto max-w-[1700px] rounded-lg border border-zinc-800 bg-zinc-950 px-3 py-10 text-center text-white shadow-sm xl:px-4 xl:py-12">
          <div className="mx-auto max-w-3xl">
            <p className="mb-2 text-[10px] font-black uppercase tracking-[0.22em] text-zinc-400">
              Nguồn Nhà Giá Rẻ
            </p>

            <h3 className="text-2xl font-black tracking-tight xl:text-4xl">
              Bạn cần ký gửi bất động sản?
            </h3>
